...
async def universal_llm_gateway(prompt: str, agent_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {'engine_used': 'Emergent LLM', 'response': resp}
...
async def n8n_flows_trigger(flow: str, payload: Optional[Dict[str, Any]] = None):
    ...
    eff = payload if (payload and len(payload) > 0) else defaults.get(flow, {})
    body = N8nTriggerUrlPayload(url=rec.get('url'), payload=eff)
...
async def n8n_diagnostics(body: N8nDiagnosticsRequest):
    ...
    try:
        ...
        return {'status': status, 'http_code': resp.status_code, 'latency_ms': latency_ms, 'hint': hint, 'details': f'Response: {details}'}
    except httpx.RequestError as e:
        latency_ms = int((time.time() - started) * 1000)
        ...
        return {'status': 'error', 'http_code': 0, 'latency_ms': latency_ms, 'hint': 'Impossible de joindre n8n (réseau/host).', 'details': str(e)}
...
async def builder_list_agents():
    ...
    if not items:
        ...
        provisional: List[BuilderAgentOut] = []
        for a in defaults:
...
async def builder_ask_gateway(body: AgentAskGatewayRequest):
    ...
    await db.audit_logs.insert_one({'_id': str(uuid.uuid4()), 'id': str(uuid.uuid4()), 'event': 'builder_agent_ask_gateway', 'agent': agent_name or body.agent_id or 'anonymous', 'prompt': body.prompt[:1000], 'engine': result.get('engine_used'), 'response': (result.get('response') or '')[:4000], 'timestamp': datetime.now(timezone.utc).isoformat()})
    return result
